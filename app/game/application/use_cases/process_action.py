...
from app.game.dto.response import GameActionResponse, GameEndingResponse, GameMessageResponse
...
class ProcessActionUseCase:
    ...
    async def execute(self, input_data: ProcessActionInput) -> ProcessActionResult:
        """유스케이스 실행."""
        # 1. Idempotency check
        if input_data.idempotency_key:
            cached = await self._check_idempotency(
                input_data.session_id, input_data.idempotency_key
            )
            if cached:
                return ProcessActionResult(response=cached, is_cached=True)
        ...
        self._validate_session(session, input_data.user_id)
    ...
    def _validate_session(self, session: GameSessionEntity, user_id: UUID) -> None:
        """세션 유효성 검증."""
    ...
    async def _handle_normal_turn(
        ...
        options = GameMasterService.extract_action_options(llm_response.content)
...
"""
    ...
    async def _cache_response(
        ...
        await self._cache.set(cache_key, json.dumps(cache_data), ttl_seconds=600)
