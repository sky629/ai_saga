...
- 첫 장면에서는 캐릭터의 외형, 목표, 현재 처지를 자연스럽게 드러내세요.
- 첫 선택지 2개 이상은 캐릭터 설정(목표, 외형, 배경, 관계)과 직접 연결하세요.
...
- `npcs_met`에는 NPC 이름 문자열만 넣으세요. 예: `["엘프", "수상한 상인"]`
- `npcs_met`에 `{{"name": "엘프"}}` 같은 객체를 넣지 마세요.
"""
...
def build_system_prompt(
    ...
    character_profile_section = character_description.strip()
    if not character_profile_section:
        character_profile_section = (
            f"- 이름: {character_name}\n- 추가 설정: 아직 공개된 정보 없음."
        )
    ...
    return SYSTEM_PROMPT_TEMPLATE.format(
        ...
        character_profile_section=character_profile_section,
