...
from app.game.infrastructure.persistence.models.game_models import GameSession, GameMessage

async def check_sessions():
    async with postgres_storage.get_domain_read_session() as db:
        print("\n--- Checking Game Sessions ---")
        result = await db.execute(select(GameSession).order_by(GameSession.started_at.desc()))
        ...
        for s in sessions:
            ...
            msg_result = await db.execute(select(GameMessage).where(GameMessage.session_id == s.id))
